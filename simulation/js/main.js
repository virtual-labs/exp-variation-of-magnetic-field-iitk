...
theeta1=Math.atan(((4*3.14*10**-7)*(0.55)*(turn)*(radius**2))/(2*(3.5*10**-5)*((realPosition**2)+(radius**2))**1.5));
...
function getResistance(id) {
  ...
  if (id === 'resistance1') {
    ...
    // document.getElementById("resistance-1-display").innerText = resistance1 + " " + "ohm";

    // document.getElementById("resistance-2-display-svg").innerHTML = resistance1 + " " + "ohm";
    resmove.setAttribute("transform", `matrix(-0.2973 0 0 0.2973 ${resistance1} 354)`);
...
const procedure = () => {
sn=sn+1;
theeta1=((Math.atan(((4*3.14*10**-7)*(1)*(turn)*(radius**2))/(2*(3.5*10**-5)*((realPosition**2)+(radius**2))**1.5)*100)));
theeta0=(theeta1*(180)/Math.PI);
theeta2=parseFloat((theeta1).toFixed(3));

// theeta2=Number( theeta2).toFixed(3);
theeta3=parseFloat((theeta1 + +generateRandomFloatInRange(0.01, 0.50)).toFixed(3));
...
theeta6=parseFloat((theeta1 + +generateRandomFloatInRange(0.01, 0.10)).toFixed(3));
...
mean=parseFloat((theeta1+ +theeta2+ +theeta3+ +theeta4+ +theeta5+ +theeta6+ +theeta7+ +theeta8)/8);
...
let value6 = generateRandomFloatInRange(0.01, 1.00);
